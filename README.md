...
🔥 리액트 독학 이후 처음 만들어본, 프로젝트 같지도 않은 프로젝트 이지만, 많은 시행착오를 겪으면서 스스로 문제를 해결할 수 있는 능력을 많이   
기르게되었습니다. Express 와 react 사이에서 연결을 잘 못했던 문제, 오타로 인해 몇시간을 공중분해 했었던 적도 많았고,  
반응형 header를 만드는 과정중에 로그인 데이터를 가져와 로그인, 로그아웃 으로 버튼을 변경하는 기능도 구현하는데 골머리를 좀 썼었습니디.  
여러가지 기억들이 남는데, 이번 프로젝트를 통해 다음번엔 좀 더 최적화를 잘 이뤄내고, Typescript 도 이용해보고싶고,  
리덕스를 더 잘 사용할 수 있도록 더 공부를 한 뒤 사용하고싶습니다.
